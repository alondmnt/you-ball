...
a tile also has to hold **several rows**, not one. a tile repeats identically, so one row in a tile means every row of the stand starts at the same x, the figures stack into vertical columns, and the crowd reads as rectangular blocks.

and shifting each row by a fixed amount is not enough either, because the shift itself repeats: the same arrangement marches across at the tile width. there is no way to vary one tile against the next - that is what a tile is - so the only lever is the **period**. the tiles are wide (373 and 271, coprime, so the two layers never come back into step) and every figure in them is placed independently: its own gap, size and colour from a deterministic sequence, rather than a row being a shifted copy of another. three rows each, and a figure overrunning the right edge is drawn again a tile-width left so the seam joins.

the drawing is free - one rasterisation at the tile size, 373x75, reused for every frame after. the cost is bytes, and this text compresses about six to one: the stylesheet goes from 17.7KB to 22.3KB gzipped. at 6x CPU throttle moon, the heaviest scene at seven layers, costs 0.8ms a frame with the crowd still and 1.0ms while it bounces - lower than the 1.0 and 1.3 of the version it replaced.
